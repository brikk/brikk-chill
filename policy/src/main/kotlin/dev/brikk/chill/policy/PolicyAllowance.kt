...
sealed class PolicyAllowance(_fqnTarget: String, val actions: Set<AccessTypes>, val validActions: Set<AccessTypes>) {
    ...
    init {
        require(actions.all { it in validActions }) {
            "Requested actions $actions are not valid for ${this::class.qualifiedName!!} ($validActions)"
        }
    }
    ...
    class PackageAccess(fqPackageName: String, actions: Set<AccessTypes>, val requireSealed: Boolean = false) :
        PolicyAllowance(fqPackageName, actions, ALL_PACKAGE_ACCESS_TYPES) {

        init {
            require(!requireSealed) { "Sealed-package policies are not supported; use explicit classes or requireSealed=false" }
        }

        val packageId = fqnTarget
    ...
    sealed class ClassLevel(fqClassName: String, actions: Set<AccessTypes>, validActions: Set<AccessTypes>) :
        PolicyAllowance(fqClassName, actions, validActions) {
        ...
        class ClassAccess(fqClassName: String, actions: Set<AccessTypes>) :
            ClassLevel(fqClassName, actions, ALL_CLASS_ACCESS_TYPES) {
            override fun sigPart(): String = ""
        }

        class ClassMethodAccess(fqClassName: String, val methodName: String, _methodSig: String, actions: Set<AccessTypes>) :
            ClassLevel(fqClassName, actions, ALL_CLASS_METHOD_ACCESS_TYPES) {
            val methodSig: String = _methodSig.replace('/', '.')
            override fun sigPart(): String = if (methodName == "*") "*" else "$methodName$methodSig"
        }

        class ClassConstructorAccess(fqClassName: String, _constructorSig: String, actions: Set<AccessTypes>) :
            ClassLevel(fqClassName, actions, ALL_CLASS_CONSTRUCTOR_ACCESS_TYPES) {
            val constructorSig: String = _constructorSig.replace('/', '.')
            override fun sigPart(): String =
                if (constructorSig == "*") "*" else "<init>:${constructorSig.substringBefore(')') + ")L${fqnTarget};"}"
        }

        class ClassFieldAccess(fqClassName: String, val fieldName: String, _fieldTypeSig: String, actions: Set<AccessTypes>) :
            ClassLevel(fqClassName, actions, ALL_CLASS_FIELD_ACCESS_TYPES) {
            val fieldTypeSig: String = _fieldTypeSig.replace('/', '.')
            override fun sigPart(): String = "$fieldName:$fieldTypeSig"
        }

        class ClassPropertyAccess(fqClassName: String, val propertyName: String, _propertyTypeSig: String, actions: Set<AccessTypes>) :
            ClassLevel(fqClassName, actions, ALL_CLASS_PROP_ACCESS_TYPES) {
