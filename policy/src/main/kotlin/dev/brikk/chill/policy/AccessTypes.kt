...
val ALL_CLASS_CONSTRUCTOR_ACCESS_TYPES = setOf(AccessTypes.call_Class_Constructor)

val ALL_CLASS_METHOD_ACCESS_TYPES = setOf(AccessTypes.call_Class_Static_Method, AccessTypes.call_Class_Instance_Method)

val ALL_CLASS_FIELD_ACCESS_TYPES = setOf(
    AccessTypes.read_Class_Static_Field, AccessTypes.write_Class_Static_Field,
    AccessTypes.read_Class_Instance_Field, AccessTypes.write_Class_Instance_Field,
)

val ALL_CLASS_PROP_ACCESS_TYPES = setOf(
...
internal val STATIC_CLASS_ACCESS_TYPES = setOf(
    AccessTypes.read_Class_Static_Field, AccessTypes.write_Class_Static_Field,
    AccessTypes.read_Class_Static_Property, AccessTypes.write_Class_Static_Property,
    AccessTypes.call_Class_Static_Method,
)

internal val INSTANCE_CLASS_ACCESS_TYPES = setOf(
    AccessTypes.call_Class_Constructor, AccessTypes.call_Class_Instance_Method,
    AccessTypes.read_Class_Instance_Field, AccessTypes.write_Class_Instance_Field,
    AccessTypes.read_Class_Instance_Property, AccessTypes.write_Class_Instance_Property,
)
...
internal fun Set<AccessTypes>.addDefaultClassActions(): Set<AccessTypes> {
    // (historical note: the original had a precedence bug that dropped ref_Class_Instance
    // whenever a static access type was also present; both are intended here)
    val temp = this +
        (if (this.any { it in STATIC_CLASS_ACCESS_TYPES }) listOf(AccessTypes.ref_Class_Static) else emptyList()) +
        (if (this.any { it in INSTANCE_CLASS_ACCESS_TYPES }) listOf(AccessTypes.ref_Class_Instance) else emptyList())
