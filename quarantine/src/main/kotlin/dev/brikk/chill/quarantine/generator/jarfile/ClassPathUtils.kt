...
internal object ClassPathUtils {
    fun findKotlinStdLibOrEmbeddedCompilerJars(classLoader: ClassLoader): List<File> {
        val stdlibCheck = listOfNotNull(Pair::class.containingClasspath(classLoader, """.*[/\\]kotlin-stdlib.*\.jar""".toRegex()))
        val embdCompilerCheck = listOfNotNull(Pair::class.containingClasspath(classLoader, """.*[/\\]kotlin-compiler-embeddable.*\.jar""".toRegex()))
        return (stdlibCheck.takeUnless { it.isEmpty() } ?: embdCompilerCheck)
            .assertNotEmpty("Cannot find kotlin stdlib classpath, which is required")
    }
